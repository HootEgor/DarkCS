...
func GetStat(log *slog.Logger, handler Core) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err := handler.GetQrStat(req.Group, req.Phone)
		if err != nil {
			logger.Error("GetStat", sl.Err(err))
			render.JSON(w, r, response.Error(fmt.Sprintf("GetStat: %v", err)))
			return
		}
